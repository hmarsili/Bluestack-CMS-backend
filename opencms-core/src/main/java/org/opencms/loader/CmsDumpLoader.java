...
public class CmsDumpLoader implements I_CmsResourceLoader {
    ...
    throws IOException, CmsException {
        ...
        boolean inCmsMediosAdmin = resource.getRootPath().startsWith("/system/modules/com.tfsla.diario.admin/");
        String typeName = OpenCms.getResourceManager().getResourceType(resource.getTypeId()).getTypeName();
        boolean mustCache = inCmsMediosAdmin && (typeName.equals("plain") || typeName.equals("image") || typeName.equals("video") || typeName.equals("external-image")  || typeName.equals("video-link"));
        
        if (mustCache)
        	m_clientCacheMaxAge =28800000;
        ...
        if (!mustCache && CmsWorkplaceManager.isWorkplaceUser(req)) {
