...
public class CmsImageScaler {
     ...
     * @cms cmsObject to obtain focalPoint property
     * @resource  resource element to read property
     */
    public CmsImageScaler(HttpServletRequest request, int maxScaleSize, int maxBlurSize, CmsObject cms, CmsResource resource) {
            ...
            if (isCropping()) {
                ...
                switch (getType()) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        // scale to exact target size, crop what does not fitf
    ...
    public String toString() {
        ...
        if (m_focalPointX!=-1 && m_focalPointY !=-1) {
        	result.append(',');
            result.append(CmsImageScaler.SCALE_PARAM_FOCALPOINT_X);
            result.append(':');
            result.append(m_focalPointX);
            result.append(',');
            result.append(CmsImageScaler.SCALE_PARAM_FOCALPOINT_Y);
            result.append(':');
            result.append(m_focalPointY);
        }
