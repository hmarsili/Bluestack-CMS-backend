...
public class CmsPublishScheduledJob implements I_CmsScheduledJob {
    ...
    public synchronized String launch(CmsObject cms, Map parameters) throws Exception {
        ...
        //mantengo la copia del cms que se recive
        //CmsObject auxCmsObjet = OpenCms.initCmsObject(cms);
        //try {
        //    String userName = (String)parameters.get(PARAM_USER);
        //    try {
    	//		cmsObject = OpenCms.initCmsObject(new CmsDefaultUsers().getUserGuest());
    	//		cmsObject.loginUser(userName);
    	...
    	//		cmsObject=null;
    	//		LOG.error("Error al intentar loguearse con el usuario: " + userName,e);
    	//	} 
		//} catch (Exception ex){
		//	CmsLog.getLog(this).error("Error al intentar obtener el cmsObject del evento",ex);
		//}
        
        //si obtengo un cmsObject uso el que tengo
		//if (cmsObject != null) 
		//	cms = cmsObject;
        //try {
        	try {
        		publishScheduled  (cms,  linkcheck,  report,  project,  finishMessage);
    ...
    public void publishScheduled (CmsObject cms, String linkcheck, CmsLogReport report, CmsProject project, String finishMessage) throws CmsException, Exception{
