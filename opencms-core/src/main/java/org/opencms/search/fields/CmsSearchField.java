...
public class CmsSearchField {
    ...
    /** Name of the field that contains the document expires date (hardcoded). */
    public static final String FIELD_DATE_EXPIRES = "expires";

    /** Name of the field that contains the document last modification date for fast lookup (hardcoded). */
    public static final String FIELD_DATE_EXPIRES_LOOKUP = "expires_lookup";

    /** Name of the field that contains the document expires date (hardcoded). */
    public static final String FIELD_DATE_RELEASED = "released";

    /** Name of the field that contains the document last modification date for fast lookup (hardcoded). */
    public static final String FIELD_DATE_RELEASED_LOOKUP = "released_lookup";
    ...
    /** Name of the field that contains if if the document is in the time range specified in the search parameters (hardcoded). */
    public static final String FIELD_TIMERANGE = "isInTimeRange";
    ...
    /** Name of the field that contains the document name in the VFS (hardcoded). */
    ...
    /** Indicates if the content of this field should by sorted */
     ...
     */
    public Field[] createFields(String content) {
        ...
        if (content != null) {
            ...
            //Para comprimir ahora hay que guardar el dato comprimido. Aparentemente lo hace en forma automatica
            // https://lucene.apache.org/core/6_3_0/core/org/apache/lucene/codecs/compressing/CompressingStoredFieldsFormat.html
            
            Field result = new Field(getName(), content, fieldType);
            ...
            if (isSortable()) {
            	if (isTokenized()) {
            		...
            		for (String line :lines) {
            			if (getType().equals(FIELD_TYPE_STRING)) {
            				fieldList.add(new SortedSetDocValuesField(getName(), new BytesRef(line.toLowerCase())));
            			} 
            			else if (getType().equals(FIELD_TYPE_NUMERIC)) {
            				try {
            					fieldList.add(new LongPoint(getName(),Long.parseLong(line)));
            					fieldList.add(new SortedNumericDocValuesField(getName(), Long.parseLong(line)));
            				} catch (NumberFormatException ex) {}
            			}
            			else if (getType().equals(FIELD_TYPE_FLOAT)) {
            				try {
            					fieldList.add(new FloatPoint(getName(),Float.parseFloat(line)));
            					fieldList.add(new SortedNumericDocValuesField(getName(), NumericUtils.floatToSortableInt(Float.parseFloat(line))));
            				} catch (NumberFormatException ex) {}
            			}
            		}
            	}
            		...
            		if (getType().equals(FIELD_TYPE_STRING)) {
        				fieldList.add(new SortedSetDocValuesField(getName(), new BytesRef(content.toLowerCase())));
        			} 
        			else if (getType().equals(FIELD_TYPE_NUMERIC)) {
        				try {
        					fieldList.add(new LongPoint(getName(),Long.parseLong(content)));
        					fieldList.add(new SortedNumericDocValuesField(getName(), Long.parseLong(content)));
        				} catch (NumberFormatException ex) {}
        			}
        			else if (getType().equals(FIELD_TYPE_FLOAT)) {
        				try {
        					fieldList.add(new FloatPoint(getName(),Float.parseFloat(content)));
        					fieldList.add(new SortedNumericDocValuesField(getName(), NumericUtils.floatToSortableInt(Float.parseFloat(content))));
        				} catch (NumberFormatException ex) {}
        			}
            	}
            }
    ...
    /**
     * Returns <code>true</code> if the content of this field is stored as sortable in the Lucene index.<p>
     *
     * @return <code>true</code> if the content of this field is stored as sortable in the Lucene index
     * 
     */
    ...
    public void setType(String type) {
    	  if (CmsStringUtil.isEmptyOrWhitespaceOnly(type)) {
	          ...
	          if (FIELD_TYPE_NUMERIC.equals(type)) {
	              m_type = FIELD_TYPE_NUMERIC;
	          } else if (FIELD_TYPE_FLOAT.equals(type)) {
	        	  m_type = FIELD_TYPE_FLOAT;
	          } else {
	        	  m_type=FIELD_TYPE_STRING;
	          }
    	  }
    ...
    /**
     * Controls if the content of this field is stored as sortable in the Lucene index.<p>
     *
     * @param sortable if <code>true</code>, then the field content is stored as sortable
     * 
     */
    ...
    /**
     * Controls if the content of this field is sortable in the Lucene index from a String parameter.<p> 
     * 
     * @param sortable if <code>"true"</code>, then the field content is stored as sortable
     * 
    ...
    public void setSortable(String sortable) {
        ...
        if (sortable != null) {
            sortable = sortable.trim().toLowerCase();
            if (STR_YES.equals(sortable)) {
                // "yes", value will be stored but not compressed
                isSortable = true;
            } else {
                // only "true" or "false" remain
                isSortable = Boolean.valueOf(sortable).booleanValue();
            }
        }
