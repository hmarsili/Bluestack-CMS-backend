...
public class CmsSearchManager implements I_CmsScheduledJob, I_CmsEventListener {
    ...
    public void initialize(CmsObject cms) throws CmsRoleViolationException {
        ...
        //TODO: Similarity.setDefault(new CmsSearchSimilarity());
    ...
    public void rebuildIndex(String indexName, I_CmsReport report) throws CmsException {
    	...
    	if (!index.getRebuildMode().equals(CmsSearchIndex.REBUILD_MODE_MANUAL)) {
    		rebuildIndexNotAuto(indexName, report);
    	}
    	else {
    		LOG.debug("Reconstruyendo indice " + indexName + " en modo MANUAL");
    		 // update the index 
     ...
     * @throws CmsException if something goes wrong
     */
    public synchronized void rebuildIndexNotAuto(String indexName, I_CmsReport report) throws CmsException {
    ...
    public void rebuildIndexes(List<String> indexNames, I_CmsReport report) throws CmsException {
    	...
    	if (!onlyAuto)
    		rebuildIndexesNoAuto(indexNames, report);
    	else  {
    ...
    public synchronized void rebuildIndexesNoAuto(List<String> indexNames, I_CmsReport report) throws CmsException {
...
*/
    ...
    protected synchronized void updateAllIndexes(CmsObject adminCms, CmsUUID publishHistoryId, I_CmsReport report) {
    	...
    	LOG.debug("updateAllIndexes " + publishHistoryId.getStringValue());
            ...
            if (!updateResources.isEmpty()) {
                ...
                while (i.hasNext()) {
                    ...
                    if (CmsSearchIndex.REBUILD_MODE_AUTO.equals(index.getRebuildMode())) {
                        ...
                        try {
                        	LOG.debug("updating index " + index.getName());
                            updateIndex(index, report, updateResources);
                            LOG.debug("end updating index " + index.getName());
    ...
    throws CmsException {
        ...
        if ((resourcesToIndex == null) || resourcesToIndex.isEmpty()) {
            ...
            try {
                ...
                if (backup != null) {
                    index.putSearcherBackup(backup);
                }
                ...
                while (sources.hasNext()) {
                    ...
                    LOG.debug("fin de updateIndex " + index.getName());
                }
                if (backup != null) {
                ...
                // oloutput finish information on the report                
                report.println(
...
LOG.debug("por obtener el index writer");
            IndexWriter writer = null;
            try {
                ...
                if (hasResourcesToUpdate) {
                    ...
                    Iterator<CmsSearchIndexUpdateData> i = updateCollections.iterator();
                    LOG.debug("por modificar el recurso en el indice");
                    while (i.hasNext()) {
                        CmsSearchIndexUpdateData updateCollection = i.next();
                    ...
                    LOG.debug("esperando hasta que termine el thread");
                    // wait for indexing threads to finish
                    while (threadManager.isRunning()) {
                        try {
