...
public class CmsVfsIndexer implements I_CmsIndexer {
    ...
    protected void addResourceToUpdateData(CmsPublishedResource pubRes, CmsSearchIndexUpdateData updateData) {
        ...
        if (pubRes.getState().isNew()) {
            ...
            if (pubRes.getPublishTag() < 0) {
            	...
            	LOG.debug(
            			"index: " + m_index.getName() + " | "
                    	+ "recurso: " + pubRes.getRootPath() + " | "
            			+ "isNew: " +  "true | "
            			+ "isIndexingExpiredContent: " + m_index.isIndexingExpiredContent() + " | "
            			+ "isResourceInTimeWindow: " + isResourceInTimeWindow(pubRes)
            			);
                if (m_index.isIndexingExpiredContent() || isResourceInTimeWindow(pubRes)) {
    ...
    throws CmsIndexException {
        ...
        if (resource.isInternal() || resource.isFolder() || (resource.getDateExpired() <= System.currentTimeMillis())) {
