...
public final class CmsPublishEngine implements Runnable {
        ...
        try {
            synchronized (this) {
                try {
                	...
                	LOG.error("Interrupted during publish engine", e);
                    // ignore
                }
            }
    ...
    protected void publishJobFinished(CmsPublishJobInfoBean publishJob) throws CmsException {
        ...
        try {
            ...
            eventData.put(I_CmsEventListener.KEY_DIRECTPUBLISH,  publishJob.isDirectPublish());
            eventData.put(I_CmsEventListener.KEY_PUBLISHLIST, publishJob.getPublishList());
            eventData.put(I_CmsEventListener.KEY_CMS_OBJECT, publishJob.getCmsObject());
