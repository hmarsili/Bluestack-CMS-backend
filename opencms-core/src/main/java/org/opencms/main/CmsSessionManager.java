...
public class CmsSessionManager {
    ...
    public void invalidateUserSessions(CmsUUID userId) {
    	if (m_sessionStorageProvider == null) {
            return ;
        }
        List<CmsSessionInfo> sessions = m_sessionStorageProvider.getAllOfUser(userId);
        for (CmsSessionInfo sessionInfo : sessions) {
	        if (sessionInfo != null) {
	            userId = sessionInfo.getUserId();
	            m_sessionStorageProvider.remove(sessionInfo.getSessionId());
	        }
