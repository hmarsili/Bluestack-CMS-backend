...
public final class OpenCmsCore {
    ...
    /** The two factor login configuration. */
    private CmsTwoFactorLoginConfiguration m_twoFactorLoginConfiguration;
    ...
    /** The token encryptation handler. */
    private I_CmsTokenEncryptationHandler m_tokenEncryptationHandler;
    ...
    protected CmsTwoFactorLoginConfiguration getCmsTwoFactorLoginConfiguration() {
    	return m_twoFactorLoginConfiguration;
    }
    ...
    protected I_CmsTokenEncryptationHandler getTokenEncryptationHandler() {

        return m_tokenEncryptationHandler;
    }
    ...
    protected CmsObject initCmsObjectFromHeader(HttpServletRequest req) throws CmsException {

        if (LOG.isDebugEnabled()) {
            LOG.debug("Trying to init cms object from session for header \"" + req.toString() + "\".");
        }

        String tokenId = req.getHeader(CmsRequestUtil.HEADER_USER_TOKENID);
        String browserId = req.getHeader(CmsRequestUtil.HEADER_BROWSER_ID);
        ...
        */
        if (tokenId == null) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("No session info found in header.");
            }
            return null;
        }
        ...
        CmsUser user = m_securityManager.loginUserByToken(cms.getRequestContext(), tokenId, browserId, remoteAddr);
		...
		*/
        ...
        String projectName = req.getHeader(CmsRequestUtil.HEADER_PROJECT);        
        if (projectName==null)
        	projectName = "Online";

        //LOG.debug("Obteniendo proyecto.");

        CmsUUID project = m_securityManager.readProject(projectName).getUuid();
        
        
        String headerSiteRoot = req.getHeader(CmsRequestUtil.HEADER_SITE);
       
        
        // initialize site root from request
        String siteroot = null;

        //LOG.debug("Comenzando a determinar siteRoot.");
        // a dedicated workplace site is configured
        if ((getSiteManager().getWorkplaceSiteMatcher().equals(site.getSiteMatcher()))) {
            // if no dedicated workplace site is configured, 
            // or for the dedicated workplace site, use the site root from the session attribute
            siteroot = headerSiteRoot;
        } else if (site.hasSecureServer()
            && getSiteManager().getWorkplaceSiteMatcher().getUrl().equals(site.getSecureUrl())) {
            // if the workplace is using the secured site
            siteroot = headerSiteRoot;
        ...
        return initCmsObject(req, user, siteroot, project, user.getOuFqn());
    }
    ...
    protected synchronized void initConfiguration(CmsParameterConfiguration configuration) throws CmsInitException {
        ...
        // read the two factor configuration
        m_twoFactorLoginConfiguration = systemConfiguration.getCmsTwoFactorLoginConfiguration();
       ...
       m_tokenEncryptationHandler = systemConfiguration.getTokenEncryptationHandler();
    ...
    protected void initMembers() {
        ...
        synchronized (LOCK) {
            ...
            m_twoFactorLoginConfiguration = new CmsTwoFactorLoginConfiguration();
    ...
    private CmsObject initCmsObject(HttpServletRequest req, HttpServletResponse res) throws IOException, CmsException {
     ...
     // first try to restore a stored session
        cms = initCmsObjectFromHeader(req);
        if (cms != null) {
            return cms;
        }
