...
public class CmsJspLoginBean extends CmsJspActionElement {
     ...
     */
    public void loginByToken(String token, String browserId) {

    	loginByToken(token, browserId, null);
    }
     ...
     */
    public void loginByToken(String token, String browserId, String projectName) {
        ...
        try {
            ...
            getCmsObject().loginUserByToken(token, browserId, getRequestContext().getRemoteAddress());
    ...
    public String TwoFactorAccessStep1(String data, String projectName) {
    	try {
    		return getCmsObject().FirstStep2FA(data);
    	} catch (CmsException e) {
            // the login has failed
            m_loginException = e;
        }
    ...
    public String TwoFactorAccessStep1(String userName, String password, String projectName) {
    	try {
    		return getCmsObject().FirstStep2FA(userName, password);
    	} catch (CmsException e) {
            // the login has failed
            m_loginException = e;
        }
    ...
    public String TwoFactorAccessGenerateCode(String tempId, boolean timeBased) {
    	try {
    		return getCmsObject().getNext2FAAccessCode(tempId,timeBased);
    	} catch (CmsException e) {
            // the login has failed
            m_loginException = e;
        }
    ...
    public void TwoFactorAccessStep2(String tempId, String code, boolean timeBased, String projectName, String redirectUri) throws IOException, CmsException {    		
    	
    	TwoFactorAccessStep2(tempId, code, timeBased, projectName, redirectUri);
        if (m_loginException == null) {
            if (redirectUri != null) {
    ...
    public void TwoFactorAccessSpecialCode(String tempId, String code, String projectName ) {
    	try {
    		CmsUser user = getCmsObject().getUser2FAByTemp(tempId);
        	
        	try {
        		//System.out.println("TwoFactorAccessStep2 -> checkInvalidLogins : " + user.getSimpleName() + " @ " + this.getRequestContext().getRemoteAddress());
        		
        		OpenCms.getLoginManager().checkInvalidLogins(user.getSimpleName(), this.getRequestContext().getRemoteAddress());
        	}
        	...
        	//System.out.println("user " + user.getSimpleName() + " - last login "+ user.getLastlogin());
        	long lastLogin = user.getLastlogin();
        	long now = System.currentTimeMillis();
        	
        	int duration = OpenCms.getCmsTwoFactorLoginConfiguration().getHoptValidTime();
        	
        	//System.out.println("duration " + duration);
        	//System.out.println("" + (now-lastLogin) + "> " + (duration*(1000*60)));
        	
        	//15 minutos entre paso 1 y 2.
        	if ((now-lastLogin)>(long)duration*(1000*60))
        		
        		m_loginException = new CmsAuthentificationException(org.opencms.security.Messages.get().container(
                        org.opencms.security.Messages.ERR_LOGIN_FAILED_TIMEOUT_1,
                        user.getSimpleName(),
                        this.getRequestContext().getRemoteAddress()));
    		
        	boolean validCode = false;
        	try {
        		validCode = getCmsObject().use2FAUniqueCode(tempId, code);
        	}
        	catch (CmsException e) {
        		m_loginException = e;
        		//System.out.println("TwoFactorAccessStep2 -> addInvalidLogin : " + user.getSimpleName() + " @ " + this.getRequestContext().getRemoteAddress());
        		OpenCms.getLoginManager().addInvalidLogin(user.getSimpleName(), this.getRequestContext().getRemoteAddress());
        	}
        	//System.out.println("user: " + user.getSimpleName() + " validCode: " + validCode);
    		if (validCode) {
    	        ...
    	        try {
    	        	 ...
    	        	 OpenCms.getLoginManager().removeInvalidLogins(user.getSimpleName(), this.getRequestContext().getRemoteAddress());
    	        	 
    	            // login the user and create a new session
    	        	user = getCmsObject().loginUser(user.getSimpleName());
    			...
    			OpenCms.getLoginManager().addInvalidLogin(user.getSimpleName(), this.getRequestContext().getRemoteAddress());
    			m_loginException = new CmsAuthentificationException(org.opencms.security.Messages.get().container(
                        org.opencms.security.Messages.ERR_LOGIN_FAILED_CODE_INVALID_1,
                        user.getSimpleName(),
                        this.getRequestContext().getRemoteAddress()));
    ...
    public void TwoFactorAccessStep2(String tempId, String code, boolean timeBased, String projectName) {
    	try {
    		CmsUser user = getCmsObject().getUser2FAByTemp(tempId);
        	...
        	boolean validCode = false;
        	try {
        		validCode = getCmsObject().validate2FACode(tempId, code, timeBased);
        	}
        	catch (CmsException e) {
        		m_loginException = e;
        		//System.out.println("TwoFactorAccessStep2 -> addInvalidLogin : " + user.getSimpleName() + " @ " + this.getRequestContext().getRemoteAddress());
    ...
    public void loginByToken(String token, String browserId, String projectName, String redirectUri) throws IOException {

    	loginByToken(token, browserId, projectName);
        if (m_loginException == null) {
            if (redirectUri != null) {
                getResponse().sendRedirect(
                    OpenCms.getLinkManager().substituteLink(getCmsObject(), redirectUri, null, true));
    ...
    public String tokenize(String browserId) {
    	try {
			return getCmsObject().createToken(getRequestContext().currentUser(), browserId);
		} catch (CmsException e) {
			m_loginException = e;
		}
    	return null;
    ...
    public void loginUserEncrypted(String data, String projectName) {
        ...
        try {
            ...
            // login the user and create a new session
        	userName = getCmsObject().loginUserEncrypted(data);
    ...
    public void logout() throws IOException {
        ...
        if (session != null) {
        	OpenCms.getSessionManager().invalidateUserSessions(getRequestContext().currentUser().getId());
            session.invalidate();
        }
