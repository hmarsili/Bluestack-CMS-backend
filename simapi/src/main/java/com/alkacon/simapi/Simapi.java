...
public class Simapi {
    ...
    /** Posicion indicator: focal point**/
    public static final int POS_FOCAL_POINT = 9;
     ...
     */
    public BufferedImage crop(BufferedImage image, int width, int height, int cropPosition,int focalPointX, int focalPointY) {
        ...
        switch (cropPosition) {
            ...
            case Simapi.POS_FOCAL_POINT:
            	if (focalPointX - width/2 <0)
            		x = 0;
            	else if (focalPointX + width/2 > imageWidth)
            		x = (imageWidth - width) < 0 ? 0: imageWidth - width;
            	else
            		x = focalPointX - width/2;
            	if (focalPointY - height/2 <0)
            		y = 0;
            	else if (focalPointY + height/2 > imageHeight)
            		y = (imageHeight - height) < 0 ? 0: imageHeight - height;
            	else
            		y = focalPointY-height/2;
            	break;
            default:
                // crop center
                x = (imageWidth - width) / 2;
     ...
     * @return the transformed image
     */
    public BufferedImage resize(BufferedImage image, int width, int height, int position,int focalPointX, int focalPointY) {
        ...
        if ((widthScale != 1.0) && (heightScale != 1.0)) {
            ...
            imageWidth = scaledImage.getWidth();
            imageHeight = scaledImage.getHeight();
            if (focalPointX != -1 && focalPointY!=-1) {
                focalPointX=(int)(focalPointX * widthScale);
            	focalPointY=(int)(focalPointY * heightScale);
            }
        } else {
            // no scale required
            scaledImage = image;
        ...
        if (focalPointX == -1 || focalPointY==-1)
        	...
        	return crop (scaledImage, width, height, position,focalPointX,focalPointY);
    }
