...
import com.tfsla.diario.vodGenericCollector.A_VodGenericCollector;
import com.tfsla.diario.vodGenericCollector.LuceneVodGenericCollector;

public class TfsVodGenericListTag extends A_XmlContentTag implements I_TfsNoticia, I_TfsCollectionListTag {
		...
		private static final Log LOG = CmsLog.getLog(TfsVodGenericListTag.class);
		TfsVOD previousVOD = null;
		TfsListaVods previousListaVods= null;

		//static private List<A_VodGenericCollector> vODCollectors = new ArrayList<A_VodGenericCollector>();
		...
		/*static {
			vODCollectors.add(new LuceneVodGenericCollector());		
		}*/
		...
		/*
		static private A_VodGenericCollector getVODCollector(Map<String,Object> parameters, String order) {
			A_VodGenericCollector bestMatchCollector = null;
			
			for (A_VodGenericCollector  collector : vODCollectors) {
		...
		*/
		...
		public void exposeVOD() {
			Object vodElement = null;
			TfsListaVods listaVods = new TfsListaVods(this.vods.size(),this.index+1,this.size, this.page);
			  
			if (m_content.toString().contains("<playlist")) {
				vodElement = new TfsPlaylist(m_cms,m_content,m_contentLocale,pageContext);
				listaVods.setCurrentPriorityZone(((TfsPlaylist)vodElement).getZonePriority());
				listaVods.setCurrentZone(((TfsPlaylist)vodElement).getZone());
			} else {
				vodElement = new TfsVOD(m_cms,m_content,m_contentLocale,pageContext);
				listaVods.setCurrentPriorityZone(((TfsVOD)vodElement).getZonePriority());
				listaVods.setCurrentZone(((TfsVOD)vodElement).getZone());
			}
			pageContext.getRequest().setAttribute("vodElement", vodElement);
		...
		protected void findVODs() {
			...
			A_VodGenericCollector collector = new LuceneVodGenericCollector(); //getVODCollector(parameters,order);
			
		 	previousVOD = (TfsVOD) pageContext.getRequest().getAttribute("vodElement");
	    	pageContext.getRequest().setAttribute("vodElement",null);
