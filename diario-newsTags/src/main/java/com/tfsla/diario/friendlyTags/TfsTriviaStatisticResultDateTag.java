...
public class TfsTriviaStatisticResultDateTag extends A_TfsTriviaCollectionValue {
	...
	@Override
	 public int doStartTag() throws JspException {
		 ...
		 setKeyName(TfsXmlContentNameProvider.getInstance().getTagName("trivia.resultDate"));
		 I_TfsCollectionListTag collection = getCurrentCollection();
		 String content = collection.getCollectionValue(getCollectionPathName()); 

		 if(content==null || (content !=null && (content.equals("") || content.equals("0")))) {
				printContent("");
		 } else{
				try {
					Date fecha = new Date(Long.parseLong(content));
