...
public class TfsEncuestasBoxTag extends BodyTagSupport implements I_TfsCollectionListTag {

	/**
	 * 
	 */
	private static final long serialVersionUID = 8815053924173549088L;
	...
	private void showEncuesta(String urlResource) throws JspException 
	{
		...
		try {
			...
			Encuesta encuesta = Encuesta.getEncuestaFromURL(cms, urlResource); //ModuloEncuestas.getCompletePath(...)
			
			if (style==null || style.trim().equals("")){
				style = encuesta.getEstilo();
				if (style==null || style.trim().equals(""))
					style = "default";
			}
			
			includeContent.setParameterToRequest("position","" + index);
			includeContent.setParameterToRequest("size","" + (encuestas==null ? 1: encuestas.size()));
			...
			if (Encuesta.ACTIVA.equals(encuesta.getEstado()))
			{
				...
				if (yaVoto || showresult.trim().toLowerCase().equals("true") || (encuesta.isUsuariosRegistrados() && user.getName().equals("Guest")) )
