...
public class TfsPlaylistTag extends A_XmlContentTag implements I_TfsNoticia, I_TfsCollectionListTag {
	...
	protected static final Log LOG = CmsLog.getLog(TfsPlaylistTag.class);

	TfsPlaylist previousPlaylist = null;
	
	
	public TfsPlaylistTag() {
		path=null;
	...
	public int doStartTag() throws JspException {
    	...
    	if (path==null) {
    		...
    		if (!m_cms.existsResource(path, CmsResourceFilter.ALL)) {
    			LOG.error("No existe la playlist solicitada: " + path );
    			return SKIP_BODY;
    		}
    		init(path);
    	}
   ...
   protected void exposePlaylist() {
    	...
    	if (playlist.getIsAutomatica().equals("true"))
    		this.m_content = playlist.getContent();	
	...
	public String getCollectionValue(String name) throws JspTagException {
		try {
			LOG.debug("TfsPlaylistTag - getCollectionValue : " + name);
	...
	public int getCollectionIndexSize(String name, boolean isCollectionPart) {
		LOG.debug("TfsPlaylistTag - getCollectionIndexSize : " + name);
		LOG.debug("TfsPlaylistTag - getCollectionIndexSize : " + name  + " --> " + getXmlDocument().getValues(name, m_locale).size());
