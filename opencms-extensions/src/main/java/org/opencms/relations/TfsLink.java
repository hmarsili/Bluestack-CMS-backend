...
public class TfsLink extends CmsLink {
    ...
    private static final Log LOG = CmsLog.getLog(TfsLink.class);
     ...
     */
    public TfsLink(Element element) {
    	super(element);
     ...
     */
    public TfsLink(String name, CmsRelationType type, CmsUUID structureId, String uri, boolean internal) {

    	super(name,type,structureId,uri,internal);
        m_element = null;
        m_name = name;
        m_type = type;
        m_internal = internal;
     ...
     * @param internal indicates if the link is internal within OpenCms 
     */
    public TfsLink(String name, CmsRelationType type, String uri, boolean internal) {
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof TfsLink) {
        	TfsLink other = (TfsLink)obj;
    ...
    public String getLink(CmsObject cms) {
        ...
        if (m_internal) {
            ...
            if ((cms.getRequestContext().getSiteRoot().length() == 0)
                ...
                // would be lost.
            	newLink = OpenCms.getLinkManager().substituteLink(cms, m_uri);
            	if (newLink.startsWith("/export/")) {
    				long dateLastModified =
    						getDateLastModified(newLink, m_uri, cms);
    		        LOG.debug("DateLastModified: " + dateLastModified + " > " + newLink + " - " + m_uri );
            ...
            newLink = OpenCms.getLinkManager().substituteLink(cms, getVfsUri(), siteRoot);
        	if (newLink.startsWith("/export/")) {
				long dateLastModified =
						getDateLastModified(newLink, m_uri, cms);
		        LOG.debug("DateLastModified: " + dateLastModified + " > " + newLink + " - " + m_uri );
	...
	public long getDateLastModified(String orig, String link,
			CmsObject cmsObject) {
