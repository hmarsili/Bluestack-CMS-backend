...
import org.opencms.workplace.tools.searchindex.CmsSearchIndexSourceAddList;

public class CmsClusterSearchIndexSourceAddList extends CmsSearchIndexSourceAddList {
   private String m_paramServer;

   public CmsClusterSearchIndexSourceAddList(CmsJspActionElement jsp) {
      super(jsp);
   }

   public CmsClusterSearchIndexSourceAddList(PageContext context, HttpServletRequest req, HttpServletResponse res) {
      this(new CmsJspActionElement(context, req, res));
   }

   public void executeListMultiActions() {
      if (this.getParamListAction().equals("maa")) {
   ...
   public void executeListSingleActions() {
      ...
      String indexsourceName = (String)item.get("cn");
      String action = this.getParamListAction();
      if (!action.equals("a") && !action.equals("cas2")) {
         this.throwListUnsupportedActionException();
      } else {
         CmsSearchIndex idx = CmsClusterRemoteCmdHelper.getSearchIndex(this.getCms(), this.getServer(), this.getParamIndexName());
   ...
   protected List getListItems() {
      ...
      List allSources = new LinkedList(CmsClusterRemoteCmdHelper.getIndexSources(this.getCms(), this.getServer()));
