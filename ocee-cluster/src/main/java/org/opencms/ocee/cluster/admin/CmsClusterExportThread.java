...
public class CmsClusterExportThread extends A_CmsReportThread {
   private Throwable m_error;
   private String m_servers = "";

   public CmsClusterExportThread(CmsObject cms, String servers) {
      super(cms, Messages.get().getBundle(cms.getRequestContext().getLocale()).key("RPT_CLUSTER_EXPORT_THREAD_NAME_0"));
   ...
   public void run() {
      ...
      try {
         this.getReport().println(Messages.get().container("RPT_CLUSTER_EXPORT_BEGIN_0"), 2);
         ...
         while(itServers.hasNext()) {
            ...
            CmsClusterManager.getInstance().getEventHandler().forwardEvent(server, CmsClusterEventTypes.UPDATE_EXPORTS.getType(), eventData, eventData);
         }

         this.getReport().println(Messages.get().container("RPT_CLUSTER_EXPORT_END_0"), 2);
