 ...
 * @since 6.0.0
 */
public class TfsContentLoadTag extends CmsJspTagContentLoad {
	 ...
	 * <p>
	 * 
	 * Initially, this is equal to the locale set using <code>{@link #setLocale(String)}</code>. However,
	 * the content locale may change in case a loaded XML content does not have the selected locale available.
	 * In this case the next default locale that is available in the content will be used as content locale.
	 * <p>
	 */
	 ...
	 * <p>
	 */
	public TfsContentLoadTag() {

		super();
	}

	/**
	 * Constructor used when using <code>contentload</code> from scriptlet code.
	 * <p>
	 * 
	 ...
	 */
	public TfsContentLoadTag(I_CmsXmlContentContainer container, PageContext context, String collectorName,
			String collectorParam, Locale locale, boolean editable) throws JspException {
	 ...
	 */
	public TfsContentLoadTag(I_CmsXmlContentContainer container, PageContext context, String collectorName,
			String collectorParam, String pageIndex, String pageSize, Locale locale, boolean editable)
			throws JspException {

		this(container, context, collectorName, collectorParam, pageIndex, pageSize, locale, CmsDirectEditMode
			.valueOf(editable));
	}
	 ...
	 */
	public TfsContentLoadTag(I_CmsXmlContentContainer container, PageContext context, String collectorName,
			String collectorParam, String pageIndex, String pageSize, Locale locale, CmsDirectEditMode editMode)
			throws JspException {
	...
	public int doStartTag() throws JspException, CmsIllegalArgumentException {
		...
		//MODIFICADO PARA SOLUCIONAR EL PROBLEMA DE CUANDO ESTA VACIO
		return this.getXmlDocument() != null ? BodyTag.EVAL_BODY_INCLUDE : BodyTag.SKIP_BODY;
	}
	...
	/**
	 * Returns <code>"true"</code> if this content load tag should only preload the values from the
	 * collector.
	 * <p>
	 * 
	 * @return <code>"true"</code> if this content load tag should only preload the values from the
	 *         collector
	 */
			...
			if (m_directEditMode.isEnabled() && (m_resourceName != null)) {
				...
				if (m_directEditFollowButtons == null) {
					...
					if (m_directEditLinkForNew == null) {
						...
						if(isAdmin()) {							
							// if create link is not null, show "edit", "delete" and "new" button for first
							// element
							...
							directEditButtons = CmsDirectEditButtonSelection.EDIT;
							// show "edit" and "delete" button for 2nd to last element
							m_directEditFollowButtons = CmsDirectEditButtonSelection.EDIT;							
						}
					}
				}
				else {
				...
				m_directEditOpen = CmsJspTagEditable.startDirectEdit(pageContext, new CmsDirectEditParams(
					m_resourceName, directEditButtons, m_directEditMode, m_directEditLinkForNew));
			}
	 ...
	 * <p>
	 * 
	 * If this is set to <code>true</code>, then the collector result will only be preloaded, but not
	 * iterated.
	 * <p>
	 * 
	...
	protected void init(I_CmsXmlContentContainer container) throws JspException {
		...
		CmsMacroResolver resolver = CmsMacroResolver
			.newInstance()
			.setCmsObject(m_cms)
		...
		try {
			...
			if (!m_preload) {
				...
				m_collectorResult = TfsContentLoadTag.limitCollectorResult(m_contentInfoBean, m_collectorResult);
	...
	//AGREGADO PARA EVITAR PONER EL BOTON DE BORRAR EN LOS NO ADMINISTRADORES
	private boolean isAdmin() {
		return this.m_cms !=null && this.m_cms.hasRole(CmsRole.ADMINISTRATOR);
	}
