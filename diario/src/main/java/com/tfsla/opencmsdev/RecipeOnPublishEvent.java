...
public class RecipeOnPublishEvent  implements I_CmsEventListener {

	private static final Log LOG = CmsLog.getLog(RecipeOnPublishEvent.class);

	public void cmsEvent(CmsEvent event) {
		int recipeType;
		try {
			recipeType = OpenCms.getResourceManager().getResourceType("receta").getTypeId();
			
			if (event.getType()==I_CmsEventListener.EVENT_BEFORE_PUBLISH_PROJECT) {

				long ahora = java.lang.System.currentTimeMillis(); 
				LOG.debug(ahora + "> - Comenzando evento recipeOnPublish.");
				...
				if (cmsObject!=null) {
					...
					for (Iterator it = pubList.getFileList().iterator();it.hasNext();) {
						...
						if (resource.getTypeId()==recipeType && resource.getState()!=CmsResource.STATE_DELETED) {
							...
							try {
								...
								CmsLog.getLog(this).error("Error al intentar modificar el contenido de la receta en la publicacion " + url,e);
							}
						} 
					}
				}
				...
				org.opencms.main.CmsLog.getLog(NewsOnPublishEvents.class).debug(ahora + "> - termine evento recipeOnPublishEvent. milisengundos: " + (despues1 - ahora));
			}
		} catch (CmsLoaderException e) {
			CmsLog.getLog(this).error("Hubo problemas publicando la receta",e);
		}
	}
	...
	private boolean setCannonical(String url, CmsXmlContent content, Locale locale, CmsObject cmsObject) {
		...
		LOG.debug("buscando canonical en la receta " + url + ": " + canonical);
		...
		if (canonical==null || canonical.trim().equals("")) {
			...
			LOG.debug("canonical en receta " + url + ": no encontrado. usando valor de titulo " + canonical);
