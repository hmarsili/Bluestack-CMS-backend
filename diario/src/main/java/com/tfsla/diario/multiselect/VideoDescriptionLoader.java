 ...
 *
 */
public class VideoDescriptionLoader extends A_DescriptionLoader  implements I_CmsEventListener {
						...
						if (url.contains("~")) {
							...
							try {
								...
								if (site!=null && !discardChanges) {
									...
									try {
										fillVideoData(cmsObject,content,"videoFlash");
									...
									try {
										fillVideoData(cmsObject,content,"videoDownload");
									} catch (CmsSecurityException ex) {
										CmsLog.getLog(this).info("El usuario no tiene suficientes permisos. Usuario: " + 
												cmsObject.getRequestContext().currentUser().getName());
										cmsObject = CmsObjectUtils.loginAsAdmin();
	...
	protected void fillVideoData(CmsObject cmsObject, CmsXmlContent content, String video) throws CmsException
	{
			...
			if (value!=null)
			{
				...
				if (pathVideo!=null && pathVideo.length()>0)
				{
					if (cmsObject.existsResource(pathVideo)) {
						...
						String descripcion = getElementValue(cmsObject,content,video,nro,"descripcion");
						String categorias = getElementValue(cmsObject,content,video,nro,"categoria");
						String keywords = getElementValue(cmsObject,content,video,nro,"keywords");
						String imagen = getElementValue(cmsObject,content,video,nro,"imagen");
						String fuente = getElementValue(cmsObject,content,video,nro,"fuente");
						String autor = getElementValue(cmsObject,content,video,nro,"autor");
						String calificacion = getElementValue(cmsObject,content,video,nro,"calificacion");
						String titulo = getElementValue(cmsObject,content,video,nro,"titulo");
						String ocultarComentario = getElementValue(cmsObject,content,video,nro,"hideComments");
						String autoplay = getElementValue(cmsObject,content,video,nro,"autoplay");
						...
						boolean mustAddCalification = mustAddProperty(cmsObject,calificacion, "video-rated",pathVideo);
						boolean mustAddHideComments = mustAddProperty(cmsObject,ocultarComentario, "hideComments",pathVideo);
						boolean mustAddAutoplay = mustAddProperty(cmsObject,autoplay,"video-autoplay",pathVideo);
						...
						if (	mustAddTitle || mustAddAgency || mustAddAuthor || mustAddCalification
								|| mustAddHideComments || mustAddDescription || mustAddKeywords
								|| mustAddCategory 	|| mustAddImage || mustAddAutoplay ) {
							...
							if(mustAddAutoplay)
								cmsObject.writePropertyObject(pathVideo, new CmsProperty("video-autoplay",autoplay, null));
