...
/**
 * Main class of the package. Instances can encode audio and video streams  (based on FFMPEG)
 */
public class Encoder {

	private static final Log LOG = CmsLog.getLog(Encoder.class);
	/**
	...
	public String[] getAudioDecoders() throws EncoderException {
		...
		try {
			...
			CmsLog.getLog(this).error("Error al obtener lista de decoders: "+e.toString());
	 ...
	 *             If a problem occurs calling the underlying ffmpeg executable.
	 */
	public LinkedHashMap<String,String> getAudioEncoders() throws EncoderException {
		
	 	LinkedHashMap<String,String> encoders = new LinkedHashMap();
		...
		try {
				...
				if (evaluate) {
					...
					if (matcher.matches()) {
						...
						if ("E".equals(encoderFlag)
								&& "A".equals(audioVideoFlag)) {
							String codec = matcher.group(4);
							
							String [] codecParts = codec.split(" ");
							String codecName = codecParts[0];
							String codecDescription = "";
							
							for( int c=1; c <= codecParts.length-1; c++){
								
								if(!codecParts[c].equals(""))
								  codecDescription = codecDescription +" "+ codecParts[c];
							}
							
							encoders.put(codecName, codecDescription);
			...
			CmsLog.getLog(this).error("Error al obtener lista de encoders: "+e.toString());
			throw new EncoderException(e);
		} finally {
			ffmpeg.destroy();
		}
	 ...
	 *             If a problem occurs calling the underlying ffmpeg executable.
	 */
	public LinkedHashMap<String, String> getVideoEncoders() throws EncoderException {
	...
	public MultimediaInfo getInfo(File source) throws InputFormatException,
		...
		try {
			...
			LOG.error("Error al ejecutar ffmpeg.execute() " + source.getAbsolutePath() , e);
			throw new EncoderException(e);
		}
		try {
			RBufferedReader reader = null;
			...
			while (true) {
				...
				if (step == 0) {
					...
					if (m.matches()) {
						...
						String format = line.replaceFirst("Input #0,", "");
					     	   format = format.substring(0,format.indexOf(", from"));
					...
					if (m.matches()) {
						...
						String[] lineParts = line.split("bitrate:");
						
						if(lineParts.length ==2)
							info.setBitrate(lineParts[1].trim());
	...
	public void automaticEncode(File source, File target, String attributes,
			EncoderProgressListener listener, long duration, ConverterLogger LOG) throws IllegalArgumentException,
			InputFormatException, EncoderException {

		if (attributes == null) {
			CmsLog.getLog(this).error("Video encoder preconfigured formats - ERROR - Both audio and video attributes are null");
			LOG.log("ERROR", "Both audio and video attributes are null");
			
			throw new IllegalArgumentException(
					"Both audio and video attributes are null");
		}
		
		LOG.log("Arguments for encoding: "+attributes);
		...
		String [] paramParts = attributes.split(" ");
		
		for(int j=0; j< paramParts.length;j++){
			ffmpeg.addArgument(paramParts[j]);
		}
		...
		try {
			...
			LOG.log("ERROR", "Error in ffmpeg execution: "+e);
			throw new EncoderException(e);
		}
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (step == 0) {
					if (line.startsWith("WARNING: ") || line.startsWith("Unable") || line.startsWith("Unrecognized") || line.startsWith("Segmentation fault")) {
						if (listener != null) {
							listener.message(line);
						}
					//} else if (!line.startsWith("Output #0")) {
						...
						if (table == null) {
							...
							if (listener != null) {
								...
								if (time != null) {
									...
									if(timeArr.length==3){
										int timeH = Integer.parseInt(timeArr[0])*60*60;
										int timeM = Integer.parseInt(timeArr[1])*60;
										
										String pS1 = timeArr[2].substring(0, timeArr[2].indexOf('.'));
										String pS2 = timeArr[2].substring(timeArr[2].indexOf('.')+1);
										int timeS = Integer.parseInt(pS1);
										int timeInt = timeH + timeM + timeS;
			...
			LOG.log("ERROR", "Error in ffmpeg encoder: "+e);
			throw new EncoderException(e);
		} finally {
			ffmpeg.destroy();
			LOG.log("ffmpeg destroy");
		}
	}
