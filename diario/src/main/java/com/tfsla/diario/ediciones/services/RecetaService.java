package com.tfsla.diario.ediciones.services;


import java.text.DecimalFormat;
import java.util.Iterator;
import java.util.List;

import org.apache.commons.logging.Log;
import org.opencms.file.CmsObject;
import org.opencms.file.CmsResource;
import org.opencms.file.CmsResourceFilter;
import org.opencms.main.CmsException;
import org.opencms.main.CmsIllegalArgumentException;
import org.opencms.main.CmsLog;
import org.opencms.main.OpenCms;

import com.tfsla.diario.ediciones.exceptions.UndefinedTipoEdicion;
import com.tfsla.diario.ediciones.model.TipoEdicion;
import com.tfsla.opencms.dev.collector.DateFolder;

public class RecetaService extends baseService {
	
	public static final String RECETA_PATH = "/recetas";
	public static final int TYPE_ID = 5069;

	
	private static final Log LOG = CmsLog.getLog(RecetaService.class);

	
	/**
	 * 
	 * Crea una receta en el directorio indicado con la numeracion correspondiente.
	 * @param obj
	 * @param location
	 * @return Path y nombre de la receta creada.
	 * @throws CmsIllegalArgumentException
	 * @throws CmsException
	 */
	public String crearRecetastNombre(CmsObject cms,String location) throws CmsIllegalArgumentException, CmsException{
		int shift=1;
		String fileName = "";
		boolean done = false;
		while (!done && shift <=10) {
			try {
				fileName = getNextRecetaName(cms,location,shift);

				int eventType = OpenCms.getResourceManager().getResourceType("receta").getTypeId();
				cms.createResource(fileName,eventType);
		
				done=true;
				
			}
			catch (CmsException ex) {
				LOG.error("Intento " + shift + " - Error al crear la receta " + fileName,ex);
				if (shift>=10)
					throw ex;
			}
			shift+=2;
		}

		return fileName;
	}
	
	public String crearRecetas(CmsObject cms, int tipoEdicion) throws CmsIllegalArgumentException, CmsException, UndefinedTipoEdicion {
		TipoEdicionService tService = new TipoEdicionService();
		TipoEdicion tEdicion = tService.obtenerTipoEdicion(tipoEdicion);
		
		String path = tEdicion.getBaseURL().replace("/contenidos/", "");
		
		path = cms.getRequestContext().removeSiteRoot(path);
		
		if(path != null && path.length() > 0 && path.endsWith("/"))
			path = path.substring(0, path.length() - 1);
		
		path +=  RECETA_PATH + "/";
		
		
		return crearRecetastNombre(cms, path);
	}
	
	private String getNextRecetaName(CmsObject obj,String location,int shift) throws CmsException {
		String fileName="";
		
		String[] niveles = location.split("/");
		
		
		//Solo si el contenido es de publicaciones de tipo ONLINE y ONLINE ROOT
		if (niveles.length <= 3)
			location = new DateFolder(location, true).getTodayFolder(obj);

		fileName = createRecetaName(obj, location,"receta", shift);

		return fileName;
	}

	private String createRecetaName(CmsObject obj, String location, String fileNamePreffix, int shift)
			throws CmsException {
		String fileName;
		int maxNewsValue  = 0;
		List cmsFiles = obj.getResourcesInFolder(location, CmsResourceFilter.ALL);
		for (Iterator it = cmsFiles.iterator(); it.hasNext();) {
			CmsResource resource = (CmsResource) it.next();
			fileName = resource.getName();
			if (fileName.matches(".*" + fileNamePreffix + "_[0-9]{4}.html")) {
				String auxFileName =fileName.substring(fileName.indexOf(fileNamePreffix + "_"));
				int newsValue = Integer.parseInt(auxFileName.replace(fileNamePreffix + "_","").replace(".html",""));
				if (maxNewsValue<newsValue)
					maxNewsValue=newsValue;
			}
		}

		DecimalFormat df = new DecimalFormat("0000");
		fileName = location + fileNamePreffix + "_" + df.format(maxNewsValue+shift) + ".html"; 
		return fileName;
	}
	
}
