...
public class TemporadasService extends baseService {
	public static final String PELICULAS_PATH = "vod";
	public static final int TYPE_ID = 5067;
	 ...
	 * @throws CmsException
	 */
	public String crearTemporadaNombre(CmsObject cms,String location) throws CmsIllegalArgumentException, CmsException{
		int shift=1;
		String fileName = "";
		boolean done = false;
		while (!done && shift <=10) {
			try {
				fileName = getNextTemporadaName(cms,location,shift);

				int eventType = OpenCms.getResourceManager().getResourceType("temporada").getTypeId();
				...
				LOG.error("Intento " + shift + " - Error al crear la temporada " + fileName,ex);
				if (shift>=10)
					throw ex;
			}
			shift+=2;
	...
	public String crearTemporada(CmsObject cms, int tipoEdicion) throws CmsIllegalArgumentException, CmsException, UndefinedTipoEdicion {
		...
		return crearTemporadaNombre(cms, path);
	}
	
	private String getNextTemporadaName(CmsObject obj,String location,int shift) throws CmsException {
		String fileName="";
		...
		fileName = createTemporadaName(obj, location,"temporada", shift);

		return fileName;
	}

	private String createTemporadaName(CmsObject obj, String location, String fileNamePreffix, int shift)
			throws CmsException {
		String fileName;
		int maxNewsValue  = 0;
