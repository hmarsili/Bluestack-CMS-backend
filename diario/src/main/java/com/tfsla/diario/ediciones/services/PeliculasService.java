...
public class PeliculasService extends baseService {
	public static final String PELICULAS_PATH = "vod";
	public static final int TYPE_ID = 5064;
	 ...
	 * @throws CmsException
	 */
	public String crearPeliculaNombre(CmsObject cms,String location) throws CmsIllegalArgumentException, CmsException{
		int shift=1;
		String fileName = "";
		boolean done = false;
		while (!done && shift <=10) {
			try {
				fileName = getNextPeliculaName(cms,location,shift);

				int eventType = OpenCms.getResourceManager().getResourceType("pelicula").getTypeId();
	...
	public String crearPeliculas(CmsObject cms, int tipoEdicion) throws CmsIllegalArgumentException, CmsException, UndefinedTipoEdicion {
		...
		return crearPeliculaNombre(cms, path);
	}
	
	private String getNextPeliculaName(CmsObject obj,String location,int shift) throws CmsException {
		String fileName="";
		...
		fileName = createPeliculaName(obj, location,"pelicula", shift);

		return fileName;
	}

	private String createPeliculaName(CmsObject obj, String location, String fileNamePreffix, int shift)
			throws CmsException {
		String fileName;
		int maxNewsValue  = 0;
