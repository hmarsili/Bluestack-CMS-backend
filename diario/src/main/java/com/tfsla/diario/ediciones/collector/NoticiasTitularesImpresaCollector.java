...
public class NoticiasTitularesImpresaCollector extends A_CmsResourceCollector {
	...
	public List getCollectorNames() {
		...
		nombres.add("NoticiasTitularesImpresa");
		return nombres;
	}
	...
	public List getResults(CmsObject cms, String collectorName, String param)
		...
		int positionZona = findSeccionPosition(content,cms);
	...
	private List<CmsResource> getNoticias(CmsXmlContent content, CmsObject cms, int positionZona) throws CmsException
	{
		...
		if (positionZona>0)
		{
			...
			{
				nroNoticia++;
				String name ="seccion[" + positionZona + "]/noticia[" + nroNoticia + "]";
	...
	private int findSeccionPosition(CmsXmlContent content, CmsObject cms)
	{
		int x=0;
		...
		{
			x++;
			String name = "seccion["+x+"]/nombre[1]";
			I_CmsXmlContentValue value = content.getValue(name, Locale.ENGLISH);
			if(value!=null)
			{
				String zonaName = value.getPlainText(cms);
				if (zonaName.equals(seccion))
					endProcess = true;
