...
import com.tfsla.diario.freshness.model.*;
...
public class FreshnessService extends baseService {
	...
	/**
	 * Freshness recibe un object con la clave principal compuesta por publicacion,
	 * siteName y url
	 * 
	 * @throws Exception
	 * @throws NumberFormatException
	 */
	public boolean hasSetFreshness(int publication, String siteName, String url)
	...
	public Freshness getFreshness(int publication, String siteName, String url)
			throws NumberFormatException, Exception {
	...
	public List<Freshness> getFreshness(int publication, Long frontDate, Long toDate)
			throws NumberFormatException, Exception {

		FreshnessDAO fDAO = new FreshnessDAO();
		return fDAO.getFreshness(publication, frontDate, toDate);
	...
	public List<Freshness> searchFresh(SearchOptions options) throws NumberFormatException, Exception {

		FreshnessDAO fDAO = new FreshnessDAO();
		return fDAO.getFreshness(options);
	...
	public void createFreshness(Freshness freshness) throws Exception {
		...
		LOG.debug("ferscura en dao " + freshness.getSection());
		FreshnessDAO fDAO = new FreshnessDAO();
	...
	public Freshness formatJsonToFreshness(JSONObject jsonRequest, String pathNew) throws Exception {
		...
		if (jsonRequest.getString("type").equals("RECURRENCE")) {
			// se comenta la siguiente linea porque la priemra ejecucion es la fecha que el
			// usuario selecciono desd el front.
		...
		LOG.debug("PROBANDO llegando" + freshness.getSection());
		freshness.setSection(jsonRequest.has("section") ? jsonRequest.getString("section") : null);
	...
	public Long setDateFreshness(Long dateNews, int frecuency) {
		...
		/**
		 * Calendar nowCal = Calendar.getInstance(); nowCal.getTime(); while
		 * (nowCal.getTime().before(calendar.getTime())) { LOG.debug("FECHA ACTUAL" +
		 * nowCal.getTime() + "FECHA NUEVA " + calendar.getTime());
		 * calendar.add(calendar.DAY_OF_YEAR, frecuency); calendar.getTime(); }
		 */
	...
	public JSONObject formatFreshnessToJSON(Freshness freshness) throws Exception {
		...
		freshnessJson.put("section", freshness.getSection());
	...
	public boolean isFreshnessEquals(Freshness freshness, Freshness freshness2) throws Exception {
		...
		if (freshness.getStartDate() != freshness2.getStartDate()) {
			...
			LOG.debug("SE FUE POR getRepublication" + freshness.getRepublication() + " - "
					+ freshness2.getRepublication());
			return false;
