...
public class PlaylistService extends baseService {
	...
	private static final Log LOG = CmsLog.getLog(PlaylistService.class);

	
	/**
	 * Crea una pelicula en el directorio indicado con la numeracion correspondiente.
	 * @param obj
	 * @param location
	 ...
	 * @throws CmsException
	 */
	public String crearPlaylistNombre(CmsObject cms,String location) throws CmsIllegalArgumentException, CmsException{
		int shift=1;
		String fileName = "";
		boolean done = false;
		while (!done && shift <=10) {
			try {
				fileName = getNextPlaylistName(cms,location,shift);

				int eventType = OpenCms.getResourceManager().getResourceType("playlist").getTypeId();
				...
				LOG.error("Intento " + shift + " - Error al crear la playlist " + fileName,ex);
				if (shift>=10)
					throw ex;
			}
			shift+=2;
	...
	public String crearPlaylists(CmsObject cms, int tipoEdicion) throws CmsIllegalArgumentException, CmsException, UndefinedTipoEdicion {
		...
		return crearPlaylistNombre(cms, path);
	}
	
	private String getNextPlaylistName(CmsObject obj,String location,int shift) throws CmsException {
		String fileName="";
		...
		fileName = createPlaylistName(obj, location,"playlist", shift);

		return fileName;
	}

	private String createPlaylistName(CmsObject obj, String location, String fileNamePreffix, int shift)
			throws CmsException {
		String fileName;
		int maxNewsValue  = 0;
