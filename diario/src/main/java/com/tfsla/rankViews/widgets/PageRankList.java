...
public class PageRankList  extends A_CmsListDialog {
	...
	int rankMode = TfsStatisticsOptions.RANK_HITS;

	public PageRankList(CmsJspActionElement jsp) {
		super( jsp, LIST_ID, Messages.get().container(Messages.GUI_VISITADOS_LIST_NAME_0), CAMPO_ORDEN,
			CmsListOrderEnum.ORDER_DESCENDING, null);
	}

	public PageRankList(CmsJspActionElement jsp, String list_id, CmsMessageContainer container, String campo_orden, CmsListOrderEnum order_descending, String object) {
		super(jsp, list_id, container, campo_orden, order_descending, object);
	}


	public PageRankList(PageContext context, HttpServletRequest req, HttpServletResponse res) {
	...
	private String addFilters()
	{
		...
		for (java.util.Iterator iter = tService.obtenerTipoEdiciones(siteName).iterator(); iter.hasNext();) {
		...
		for (java.util.Iterator iter = eService.obtenerEdiciones(tipoEdicion).iterator(); iter.hasNext();) {
		...
		try {
			...
			for (java.util.Iterator iter = sDAO.getSeccionesByTipoEdicionId(tipoEdicion).iterator(); iter.hasNext();) {
		...
		//TODO: Agregar los tipos de contenido para filtrar
		filterHTML+="Tipo Contenido:";
		filterHTML+="<select name=\"tContenido\" id=\"tContenido\">\n";
		
		String tCont = tipoContenido;
		if (tCont==null) tCont="";
		...
		for (Iterator it = dataCollectorManager.getDataCollectors().iterator();it.hasNext();)
		{
			...
			if (dCollector.getContentName()!=null)
				filterHTML+="	<option value=\"" + dCollector.getContentType() + "\" " + (tCont.equals(dCollector.getContentType()) ? "selected " : " ") + ">" + dCollector.getContentName() + "</option>\n";
		}
		filterHTML+="</select>\n";
		...
		filterHTML+="	<option value=\"" + TfsStatisticsOptions.RANK_HITS + "\" " + (rankMode == TfsStatisticsOptions.RANK_HITS ? "selected " : " ") + ">Hits</option>\n";
		filterHTML+="	<option value=\"" + TfsStatisticsOptions.RANK_COMENTARIOS + "\" " + (rankMode == TfsStatisticsOptions.RANK_COMENTARIOS ? "selected " : " ") + ">Comentarios</option>\n";
	...
	protected void getParameters(HttpServletRequest req)
	{
		...
		if (req.getParameter("cantidad")!=null)
		{
			...
			tipoContenido = req.getParameter("tContenido");
			if (tipoContenido!=null && tipoContenido.length()==0)
				tipoContenido=null;
	...
	protected void setColumns(CmsListMetadata metadata) {
		addColumn(metadata, "url", Messages.VISITADOS_LIST_URL_NOTICIA_COLUMN);
		addColumn(metadata, "Titulo", Messages.VISITADOS_LIST_TITULO_NOTICIA_COLUMN);
		addColumn(metadata, "seccion", Messages.VISITADOS_LIST_SECCION_NOTICIA_COLUMN);
		addColumn(metadata, "tipo", Messages.VISITADOS_LIST_TIPO_NOTICIA_COLUMN);
		addColumn(metadata, "fechaUltimaModificacion", Messages.VISITADOS_LIST_FECHA_NOTICIA_COLUMN);
		addColumn(metadata, "valor", Messages.VISITADOS_LIST_CANTIDAD_NOTICIA_COLUMN);		
	}
	...
	protected TfsStatisticsOptions getOptions()
	{
        ...
        if (tipoContenido!=null && tipoContenido.trim().equals(""))
            options.setTipoContenido(null);
        else        	
	...
	protected void addItem(List<CmsListItem> results, TfsHitPage masVisitado) throws CmsException
	{
		...
		case TfsStatisticsOptions.RANK_VALORACIONES_NEGATIVO:
			...
			break;
		case TfsStatisticsOptions.RANK_COMENTARIOS:
			listItem.set("valor",masVisitado.getComentarios());
			break;
		case TfsStatisticsOptions.RANK_RECOMENDACION:
