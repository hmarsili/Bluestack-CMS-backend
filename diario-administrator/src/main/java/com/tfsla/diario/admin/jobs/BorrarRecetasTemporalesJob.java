...
public class BorrarRecetasTemporalesJob  implements I_CmsScheduledJob {

		private String resultados = "";
		private boolean doit = true;
		protected static final Log LOG = CmsLog.getLog(BorrarRecetasTemporalesJob.class);
		...
		public String launch(CmsObject cms, Map parameters) throws Exception {
			...
			resultados += "Lanzando eliminacion de recetas  temporales entre los ultimos " + daysAfter + " y " + daysBefore + " dias\n";
			...
			for (int j=daysAfter;j<=daysBefore;j++) {
				...
				if (cms.existsResource(eventFolder))
					resuourcesToPublish.addAll(borrarEventosTemporales(cms,eventFolder));
			...
			if (resuourcesToPublish.size()>0)
				despublicarEventosBorrados(cms,resuourcesToPublish);
		...
		public List<CmsFile> borrarEventosTemporales(CmsObject cmsObject, String pathFolder) throws CmsException {
			...
			for (CmsFile file : files) {
				try {
					...
					LOG.trace("error en receta " + cmsObject.getSitePath(file),e);
				}
			}
			return resourcesToPublish;
		}
		
		public void despublicarEventosBorrados(CmsObject cmsObject, List<CmsFile> resourcesToPublish) throws CmsException {
