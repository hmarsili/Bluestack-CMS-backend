...
public class BorrarTriviasTemporalesJob  implements I_CmsScheduledJob {

		private String resultados = "";
		private boolean doit = true;
		protected static final Log LOG = CmsLog.getLog(BorrarTriviasTemporalesJob.class);
		...
		public String launch(CmsObject cms, Map parameters) throws Exception {
			...
			resultados += "Lanzando eliminacion de trivias  temporales entre los ultimos " + daysAfter + " y " + daysBefore + " dias\n";
			...
			for (int j=daysAfter;j<=daysBefore;j++) {
				...
				if (cms.existsResource(triviaFolder))
					resuourcesToPublish.addAll(borrarEventosTemporales(cms,triviaFolder));
		...
		public List<CmsFile> borrarEventosTemporales(CmsObject cmsObject, String pathFolder) throws CmsException {
			...
			for (CmsFile file : files) {
				try {
					...
					LOG.trace("error en trivias " + cmsObject.getSitePath(file),e);
				}
			}
		...
		public void despublicarEventosBorrados(CmsObject cmsObject, List<CmsFile> resourcesToPublish) throws CmsException {
			resultados += "Despublicando las trivias eliminadas.\n";
