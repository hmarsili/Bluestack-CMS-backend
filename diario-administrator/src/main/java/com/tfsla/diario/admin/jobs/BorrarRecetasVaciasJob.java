...
public class BorrarRecetasVaciasJob implements I_CmsScheduledJob {

	private String resultados = "";
	private boolean doit = true;
	protected static final Log LOG = CmsLog.getLog(BorrarRecetasVaciasJob.class);
	...
	public String launch(CmsObject cms, Map parameters) throws Exception {
		...
		resultados += "Lanzando eliminacion de recetas vacias entre los ultimos " + daysAfter + " y " + daysBefore + " dias\n";
		...
		for (int j=daysAfter;j<=daysBefore;j++) {
			...
			recipeFolder = baseUrl + folderDateFormat.format(cal.getTime()) + "/";

			resultados += "revisando carpeta " + recipeFolder + ".\n";

			if (cms.existsResource(recipeFolder))
				resuourcesToPublish.addAll(borrarRecetasVacias(cms,recipeFolder));
			
			cal.add(Calendar.DAY_OF_MONTH, -1);
		}
		
		if (resuourcesToPublish.size()>0)
			despublicarRecetasBorradas(cms,resuourcesToPublish);
	...
	public List<CmsFile> borrarRecetasVacias(CmsObject cmsObject, String pathFolder) throws CmsException {
		...
		for (CmsFile file : files) {
			try {
				resultados += "Eliminando receta " + cmsObject.getSitePath(file) + ".\n";
				...
				LOG.info("Error cuando intenga obtener las recetas a borrar", e);
				System.out.println("error en receta " + cmsObject.getSitePath(file));
				e.printStackTrace();
			}
		}
	...
	public void despublicarRecetasBorradas(CmsObject cmsObject, List<CmsFile> resourcesToPublish) throws CmsException {
		resultados += "Despublicando las recetas eliminados.\n";
