...
public class BorrarVodTemporalesJob  implements I_CmsScheduledJob {
		...
		public String launch(CmsObject cms, Map parameters) throws Exception {
			...
			resultados += "Lanzando eliminacion de eventos  temporales entre los ultimos " + daysAfter + " y " + daysBefore + " dias\n";
			...
			for (int j=daysAfter;j<=daysBefore;j++) {
				...
				vodFolder = baseUrl + folderDateFormat.format(cal.getTime()) + "/";
				
				resultados += "revisando carpeta " + vodFolder + ".\n";
				
				if (cms.existsResource(vodFolder))
					resuourcesToPublish.addAll(borrarVodTemporales(cms,vodFolder));
			...
			if (resuourcesToPublish.size()>0)
				despublicarVodBorrados(cms,resuourcesToPublish);
		...
		public List<CmsFile> borrarVodTemporales(CmsObject cmsObject, String pathFolder) throws CmsException {
			...
			for (CmsFile file : files) {
				try {
					resultados += "Eliminando evento " + cmsObject.getSitePath(file) + ".\n";
		...
		public void despublicarVodBorrados(CmsObject cmsObject, List<CmsFile> resourcesToPublish) throws CmsException {
			resultados += "Despublicando los eventos eliminados.\n";
