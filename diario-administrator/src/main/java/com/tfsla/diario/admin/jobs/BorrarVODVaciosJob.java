...
public class BorrarVODVaciosJob implements I_CmsScheduledJob {
	...
	public String launch(CmsObject cms, Map parameters) throws Exception {
		...
		resultados += "Lanzando eliminacion de Vod  vacios entre los ultimos " + daysAfter + " y " + daysBefore + " dias\n";
		...
		for (int j=daysAfter;j<=daysBefore;j++) {
			...
			if (cms.existsResource(vodFolder))
				resuourcesToPublish.addAll(borrarVODsVacios(cms,vodFolder));
	...
	public List<CmsFile> borrarVODsVacios(CmsObject cmsObject, String pathFolder) throws CmsException {
		...
		for (CmsFile file : files) {
			try {
				...
				LOG.info("Error cuando intenga obtener los vods a borrar", e);
	...
	public void despublicarVODsBorrados(CmsObject cmsObject, List<CmsFile> resourcesToPublish) throws CmsException {
		resultados += "Despublicando los vods eliminados.\n";
