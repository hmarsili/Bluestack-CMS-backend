...
public class BorrarNoticiasCablerasJob implements I_CmsScheduledJob {
	...
	public String launch(CmsObject cms, Map parameters) throws Exception {
		...
		String _folders = (String)parameters.get("folders");
		
		String[] folders = _folders.split(",");
		...
		for (int j=daysAfter;j<=daysBefore;j++)
		{
			for(String folder : folders){
				String newsFolder = "";			
				newsFolder = folder +"/" + folderDateFormat.format(cal.getTime()) + "/";
				resultados += "revisando carpeta " + newsFolder + ".\n";
				if (cms.existsResource(newsFolder)){
					resuourcesToPublish.addAll(borrarNoticiasVacias(cms,newsFolder));	
				}
			}
