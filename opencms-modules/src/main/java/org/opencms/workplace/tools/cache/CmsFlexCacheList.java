...
public class CmsFlexCacheList extends A_CmsListDialog {
    ...
    /** list item detail id constant. */
    //public static final String LIST_SHOW_VARIATIONS_COUNT = "sv";

    /** list column id constant. */
    public static final String LIST_COLUMN_VARIATIONS = "cv";
    ...
    protected List getListItems() {
        ...
        while (itResources.hasNext()) {
            ...
            item.set(LIST_COLUMN_KEY, cache.getCachedKey(resource, getCms()).toString());
                        
            if (cache.getCachedVariations(resource, getCms())!=null)
            	item.set(LIST_COLUMN_VARIATIONS, new Integer(cache.getCachedVariations(resource, getCms()).size()));
            else 
            	item.set(LIST_COLUMN_VARIATIONS, 0);
            ret.add(item);
        }
    ...
    protected void setColumns(CmsListMetadata metadata) {
     ...
     // create column for cache key
        ...
        CmsListRegExpFormatter formatterKey = new CmsListRegExpFormatter();
        formatterKey.addRegExpStyle("never", "font-weight: bold;color:#FF0000");
        keyCol.setFormatter(formatterKey);
        // add it to the list definition
        metadata.addColumn(keyCol);
        
        
        // create column for cache key
        CmsListColumnDefinition varCol = new CmsListColumnDefinition(LIST_COLUMN_VARIATIONS);
        varCol.setName(Messages.get().container(Messages.GUI_IMAGECACHE_DETAIL_VARIATIONS_NAME_0));
        varCol.setWidth("5%");
        CmsListRangeFormatter formatter = new CmsListRangeFormatter("color:#000000");
        formatter.addRange(50,"font-weight: bold;color:#CCCC00");
        formatter.addRange(100,"font-weight: bold;color:#FF0000");
        varCol.setFormatter(formatter);

        // add it to the list definition
        metadata.addColumn(varCol);
    }
